...
A picture earns its place only when it shows something words can't — shape, structure, direction, relationship, geometry. This skill produces ONE such picture, guarantees it is **correct** (the maker renders it and looks at it before returning), and drops it into the lesson so it renders inline in Obsidian.
...
Every visual belongs to one topic. Before delegating, resolve the active topic root from the lesson or planning artifact being worked on. A topic root is the directory directly under `topics/` (for example, `topics/ai-eng`). Its visual output directory is always `<topic-root>/viz`. Never use a repository-level `viz/` directory.
...
Two makers are defined under `.agents/agents/`:
...
Delegate to the appropriate maker using the current harness's sub-agent mechanism:

```
agent: mermaid-maker
task:
  output_directory: <absolute path to the active topic root>/viz
...
`output_directory` is mandatory. Resolve it before calling the maker; do not make the maker infer the active topic from conversation context. Verify that the path is the `viz` child of the intended topic root. If the active topic cannot be identified unambiguously, ask for or discover the topic root before delegating.

The maker uses the file, rendering, and image-inspection capabilities available in its harness. It authors the source, renders it to a PNG, **looks at the PNG and iterates until it is correct and clean**, publishes it into the vault with a unique filename, and returns:

```
RESULT:
filename: viz-<slug>-<timestamp>.png
path: <absolute-topic-root>/viz/viz-<slug>-<timestamp>.png
```
...
Obsidian resolves the embed by filename anywhere in the vault because the maker saves into the active topic's `viz` folder. Width `|500` is a good default; use larger for dense diagrams. Introduce the visual in a sentence, then let it carry the idea — don't narrate every element back in prose.
...
The rendering implementation is harness-dependent. You don't render anything yourself — you brief the maker and embed the filename it returns.
